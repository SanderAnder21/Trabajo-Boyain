...
• Reintegraciones cromáticas respetuosas`,
    ...
    renderProject() {
        ...
        document.getElementById('architectName').textContent = this.project.architect.name;
        document.getElementById('architectAvatar').src = this.project.architect.avatar;
        document.getElementById('architectSpecialty').textContent = this.project.architect.specialty;
    ...
    setup3DViewer() {
        ...
        const modelViewer = document.getElementById('3d-pane');
        modelViewer.innerHTML = `
            <div class="model-viewer-container">
                <div class="model-placeholder">
                    <div class="model-icon">🎮</div>
                    <h3>Visor 3D</h3>
                    <p>Modelo: ${this.project.model3d.file}</p>
                    <p>Formato: ${this.project.model3d.format.toUpperCase()}</p>
                    <div class="model-controls">
                        <button class="control-btn" onclick="alert('Rotando modelo...')">🔄 Rotar</button>
                        <button class="control-btn" onclick="alert('Acercando...')">➕ Zoom In</button>
                        <button class="control-btn" onclick="alert('Alejando...')">➖ Zoom Out</button>
                        <button class="control-btn" onclick="alert('Vista resetada...')">⭕ Reset</button>
                    </div>
                    <div class="model-info">
                        <p><strong>Nota:</strong> En una implementación real, aquí se cargaría el modelo 3D usando Three.js u otra librería.</p>
                    </div>
                </div>
            </div>
        `;
    ...
    contactArchitect() {
        const subject = `Interés en proyecto: ${this.project.title}`;
        const body = `Hola ${this.project.architect.name},\n\nEstoy interesado/a en tu proyecto "${this.project.title}" y me gustaría obtener más información.\n\nSaludos cordiales.`;
        
        // Simular envío de email (en producción sería un formulario real)
        alert(`Esta función enviaría un email a: ${this.project.architect.contact}\nAsunto: ${subject}\n\nMensaje: ${body}`);
    }
